...
;; - Locks minting to xtrata-v3.0.0.
...
;; - Uses xtrata-v3 core-native single-tx mint paths for small files.
;; - Adds optional project/page metadata fields and cover inscription reference.
;; - Separates dependencies from optional parent relationships.
...
(define-constant ALLOWED-XTRATA-CONTRACT .xtrata-v3-0-0)
...
(define-trait xtrata-trait
  (
    ...
    (seal-with-relationships ((buff 32) (string-ascii 256) (list 50 uint) (list 50 uint)) (response uint uint))
    (mint-single-tx ((buff 32) (string-ascii 64) uint (list 50 (buff 16384)) (string-ascii 256)) (response uint uint))
    (mint-single-tx-recursive ((buff 32) (string-ascii 64) uint (list 50 (buff 16384)) (string-ascii 256) (list 50 uint)) (response uint uint))
    (mint-single-tx-with-relationships ((buff 32) (string-ascii 64) uint (list 50 (buff 16384)) (string-ascii 256) (list 50 uint) (list 50 uint)) (response uint uint))
...
(define-data-var collection-project-uri (string-ascii 256) "")
(define-data-var collection-page-uri (string-ascii 256) "")
(define-data-var cover-inscription-id (optional uint) none)
...
(define-private (complete-mint-session
  (token-id uint)
  (owner principal)
  ...
  (begin
    (try! (pay-splits mint-cost))
    (map-delete MintSessions { owner: owner, hash: expected-hash })
    (var-set reserved-count (- (var-get reserved-count) u1))
    (var-set minted-count (+ (var-get minted-count) u1))
    (record-mint-for owner phase-id)
    (record-minted-token token-id owner phase-id)
    (ok token-id)
  )
)
...
(define-public (set-project-uri (project-uri (string-ascii 256)))
  (begin
    (try! (assert-config-admin))
    (try! (assert-not-finalized))
    (var-set collection-project-uri project-uri)
    (ok true)
  )
)

(define-public (set-collection-page-uri (page-uri (string-ascii 256)))
  (begin
    (try! (assert-config-admin))
    (try! (assert-not-finalized))
    (var-set collection-page-uri page-uri)
    (ok true)
  )
)

(define-public (set-cover-inscription (token-id uint))
  (begin
    (try! (assert-config-admin))
    (try! (assert-not-finalized))
    (var-set cover-inscription-id (some token-id))
    (ok true)
  )
)

(define-public (clear-cover-inscription)
  (begin
    (try! (assert-config-admin))
    (try! (assert-not-finalized))
    (var-set cover-inscription-id none)
    (ok true)
  )
)
  ...
  (begin
    ...
    (match (map-get? MintSessions { owner: tx-sender, hash: expected-hash })
      session (let (
        ...
        (complete-mint-session
          token-id
          tx-sender
          expected-hash
          (get phase-id session)
          (get mint-price session)
        )
      )
      ERR-NOT-FOUND
    )
...
(define-public (mint-seal-with-relationships
  (xtrata-contract <xtrata-trait>)
  (expected-hash (buff 32))
  (token-uri-string (string-ascii 256))
  ...
  (begin
    ...
    (match (map-get? MintSessions { owner: tx-sender, hash: expected-hash })
      session (let (
        (resolved-token-uri (resolve-token-uri-for-hash expected-hash token-uri-string))
        (dependencies (var-get default-dependencies))
        (token-id (try! (contract-call? xtrata-contract
...
(define-public (mint-seal-recursive-with-relationships
  (xtrata-contract <xtrata-trait>)
  (expected-hash (buff 32))
  (token-uri-string (string-ascii 256))
  ...
  (let ((defaults (var-get default-dependencies)))
    (begin
      ...
      (match (map-get? MintSessions { owner: tx-sender, hash: expected-hash })
        session (let (
          (resolved-token-uri (resolve-token-uri-for-hash expected-hash token-uri-string))
          (token-id (if (> (len parents) u0)
            (try! (contract-call? xtrata-contract
              seal-with-relationships
              expected-hash
              resolved-token-uri
              dependencies
  ...
  (let (
    ...
    (begin
      ...
      (let ((session (try! (ensure-mint-session expected-hash))))
        (let (
          (resolved-parents (default-to (list) parents))
          (token-id
            (if (> (len resolved-parents) u0)
              (try! (contract-call? xtrata-contract
...
(define-public (mint-small-single-tx-with-relationships
  (xtrata-contract <xtrata-trait>)
  (expected-hash (buff 32))
  (mime (string-ascii 64))
  (total-size uint)
...
(define-public (mint-small-single-tx-recursive-with-relationships
  (xtrata-contract <xtrata-trait>)
  (expected-hash (buff 32))
  (mime (string-ascii 64))
  (total-size uint)
...
(define-read-only (get-collection-metadata)
  (ok {
    ...
    project-uri: (var-get collection-project-uri),
    collection-page-uri: (var-get collection-page-uri),
    cover-inscription-id: (var-get cover-inscription-id),
    reveal-block: (var-get reveal-block)
  })
)

(define-read-only (get-project-uri)
  (ok (var-get collection-project-uri))
)

(define-read-only (get-collection-page-uri)
  (ok (var-get collection-page-uri))
)

(define-read-only (get-cover-inscription)
  (ok {
    core-contract: ALLOWED-XTRATA-CONTRACT,
    token-id: (var-get cover-inscription-id)
  })
)
...
(define-read-only (get-collection-summary)
  (ok {
    name: (var-get collection-name),
    symbol: (var-get collection-symbol),
    base-uri: (var-get collection-base-uri),
    ...
    locked-core-contract: ALLOWED-XTRATA-CONTRACT,
    max-small-mint-chunks: MAX-SMALL-MINT-CHUNKS
  })
)

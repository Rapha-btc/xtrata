;; ==========================================================================
;; RENDEZVOUS PROPERTY TESTS for leo-fakfun-xtrata  (rv `test` mode)
;; Appended by leo-fakfun-xtrata.build.sh after the invariants block.
;;
...
;; token exists. In simnet the source leo-cats has no mints, so even a canonical
...
;; The test above stops at ERR-NO-SUCH-TOKEN: the simnet leo-cats has no
...
;; the DEEPEST path by first minting leo-cat #1 as the DEPLOYER, then seeding its
...
(define-public (test-inscribe-positive-path-no-leak (expected-hash (buff 32)))
  (begin
    ...
    ;; try to mint leo-cat #1 as deployer; if the source sale is closed (simnet),
    ;; SKIP rather than fail -- the positive path is simply unreachable here.
    (if (is-none (unwrap! (contract-call? .leo-cats get-owner u1) (err u920)))
      (match (contract-call? .leo-cats claim)
        ok-v true
        err-v (asserts! false (ok true))) ;; mint blocked (sale closed) -> skip
      true)
    ;; if we reach here, #1 has an owner: drive the deep funds-in path.
    (asserts!
      (is-some (unwrap! (contract-call? .leo-cats get-owner u1) (err u921)))
      (err u922))

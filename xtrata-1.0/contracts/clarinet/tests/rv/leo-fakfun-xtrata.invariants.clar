;; ==========================================================================
;; RENDEZVOUS INVARIANTS for leo-fakfun-xtrata
;; Appended by leo-fakfun-xtrata.build.sh to the local-alias fuzz build
;; (MASTER -> .xtrata-v3-2-3, SOURCE -> .leo-cats, free-threshold -> 0 so
;; fee-for always exercises the fee/discount regime).
;;
;; leo-fakfun-xtrata == xtrata-fakfun-forever-v2 EXCEPT for `inscribe`:
;;   - master fee is read live: (get total-fee (MASTER quote-single-tx-fee ...))
;;   - the contract is FUNDED first: (stx-transfer? master-fee tx-sender
;;     current-contract)
;;   - the xtrata is minted via (as-contract? ((with-stx master-fee))
;;     (MASTER mint-single-tx ...)) -- the vault pays the master, then escrows
;;     the freshly minted twin, instead of forever-v2's mint-to-holder-then-pull.
;; All admin / gate / freeze / binding logic is unchanged.
;;
...
;;
;; quote-single-tx-fee is a READ-ONLY on the master, so it cannot revert: the
;; live-fee read and the funds-in (stx-transfer? master-fee tx-sender
;; current-contract) DO execute in simnet. The master mint still reverts: the
;; master is paused for non-owner callers, and even for the owner the chunks
;; never hash to the seeded expected-hash (a preimage the fuzzer cannot forge).
;; So inscribe ALWAYS reverts here, atomically rolling back the funds-in, leaving
;; Bindings empty, inscribed-count pinned at 0, and the contract STX balance at
;; 0. That makes the binding-integrity invariants (1,2,4,5,7) the STRONGEST form
;; of the gate claim: "the gate provably refuses to create any binding it should
;; not", and adds the NO-LEAK claim specific to this variant's funds-in step:
;; the contract never accrues STX from a reverted inscribe. The full happy-path
;; escrow/swap custody is covered by the stxer mainnet-fork sims.
...
;; inscribed-count is bumped ONLY inside inscribe, after the live-fee read, the
;; funds-in transfer, the master mint, and the map-insert into Bindings all
;; succeed. In simnet that path can never complete (master mint reverts), so the
;; gate must keep inscribed-count pinned at 0 regardless of the inscribe args RV
...
;; ---- Invariant 8 (NEW, variant-specific): STX no-leak --------------------
;; The new inscribe funds the contract BEFORE the master mint:
;;   (stx-transfer? master-fee tx-sender current-contract)
;; If the master mint then reverts, atomicity must roll the funds-in back. In
;; simnet inscribe ALWAYS reverts (the master mint never completes), so the
;; contract's STX balance must stay exactly 0 -- no master-fee STX may ever be
;; stranded in the vault by a half-completed inscribe. A funds-in transfer whose
;; matching out-transfer (the as-contract? with-stx mint) failed to roll back
;; would leave a positive balance here and trip this invariant. (No swap path
;; moves STX, and no admin path moves STX, so 0 is the only legitimate value.)
(define-read-only (invariant-no-stranded-stx)

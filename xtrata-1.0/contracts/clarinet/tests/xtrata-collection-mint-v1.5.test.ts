...
const coreContract = `${deployer}.xtrata-v3-0-0`;
const legacyCoreContractPrincipal = Cl.contractPrincipal(deployer, 'xtrata-v1-1-0');
const mintContract = `${deployer}.xtrata-collection-mint-v1-5`;
const coreContractPrincipal = Cl.contractPrincipal(deployer, 'xtrata-v3-0-0');
const mintContractPrincipal = Cl.contractPrincipal(deployer, 'xtrata-collection-mint-v1-5');
...
function mintSmallSingleTxWithRelationships(
  sender: string,
  hash: string,
  chunksHex: string[],
  tokenUri: string,
  ...
  return simnet.callPublicFn(
    ...
    [
      ...
      Cl.list(chunksHex.map((chunkHex) => Cl.bufferFromHex(chunkHex))),
      Cl.stringAscii(tokenUri),
      Cl.list(parents.map((parent) => Cl.uint(parent)))
    ],
    sender
  ).result;
}
...
function getMintOrigin(tokenId: bigint, sender: string) {
  return simnet.callReadOnlyFn(coreContract, 'get-mint-origin', [Cl.uint(tokenId)], sender).result;
}
...
describe('xtrata-collection-mint-v1.5 recipient access controls', () => {
  ...
  it('keeps mint begin/chunk/seal flow working with v1.5 controls', () => {
    configureMintForTests();

    const hash = computeFinalHash(['aa']);
    unwrapOk(beginMint(wallet1, hash));
    unwrapOk(addChunk(wallet1, hash, 'aa'));
    const sealResult = sealMint(wallet1, hash, 'data:text/plain,v1.5');
    expect(sealResult.type).toBe(ClarityType.ResponseOk);
  });
});

describe('xtrata-collection-mint-v1.5 single-tx small flow', () => {
  ...
  it('allows repeated content while keeping collection accounting consistent', () => {
    configureMintForTests();
    ...
    expect(duplicateResult.type).toBe(ClarityType.ResponseOk);
  ...
  it('records mint origin and relationship links for collection mints', () => {
    configureMintForTests();

    const parentId = mintCoreDirect(wallet1, ['03'], 'data:text/plain,parent');
    const childHash = computeFinalHash(['04']);
    const childId = unwrapOkUint(
      mintSmallSingleTxWithRelationships(
        wallet1,
        childHash,
        ['04'],
        'data:text/plain,child',
    ...
    expect(getMintOrigin(childId, wallet1)).toBeSome(mintContractPrincipal);
    expect(simnet.callReadOnlyFn(coreContract, 'get-parents', [Cl.uint(childId)], wallet1).result).toEqual(
      Cl.list([Cl.uint(parentId)])
    );
  });

  it('stores optional collection links and exposes them in summary readers', () => {
    unwrapOk(
      simnet.callPublicFn(
        mintContract,
        'set-project-uri',
        [Cl.stringAscii('https://example.com/project')],
        deployer
      ).result
    );
    unwrapOk(
      simnet.callPublicFn(
        mintContract,
        'set-collection-page-uri',
        [Cl.stringAscii('https://example.com/collection/drop')],
        deployer
      ).result
    );
    unwrapOk(simnet.callPublicFn(mintContract, 'set-cover-inscription', [Cl.uint(7)], deployer).result);

    expect(simnet.callReadOnlyFn(mintContract, 'get-project-uri', [], deployer).result).toBeOk(
      Cl.stringAscii('https://example.com/project')
    );
    expect(
      simnet.callReadOnlyFn(mintContract, 'get-collection-page-uri', [], deployer).result
    ).toBeOk(Cl.stringAscii('https://example.com/collection/drop'));
    expect(simnet.callReadOnlyFn(mintContract, 'get-cover-inscription', [], deployer).result).toBeOk(
      Cl.tuple({
        'core-contract': coreContractPrincipal,
        'token-id': Cl.some(Cl.uint(7))
      })
    );
    expect(simnet.callReadOnlyFn(mintContract, 'get-collection-summary', [], deployer).result).toBeOk(
      Cl.tuple({
        name: Cl.stringAscii(''),
        symbol: Cl.stringAscii(''),
        'base-uri': Cl.stringAscii(''),
        description: Cl.stringAscii(''),
        'project-uri': Cl.stringAscii('https://example.com/project'),
        'collection-page-uri': Cl.stringAscii('https://example.com/collection/drop'),
        'cover-inscription-id': Cl.some(Cl.uint(7)),
        'reveal-block': Cl.uint(0),
        paused: Cl.bool(true),
        ...
        'locked-core-contract': coreContractPrincipal,
        'max-small-mint-chunks': Cl.uint(30)
      })
    );
  });
});

...
(define-map MintSessions
  ...
  { fee-paid: bool, phase-id: uint, created-at: uint }
)
          ...
          (if (> phase-id u0)
            (let ((phase (try! (assert-phase-active phase-id))))
              (begin
                ...
                (try! (pay-splits (get mint-price phase)))
                (var-set reserved-count (+ (var-get reserved-count) u1))
                (record-reservation tx-sender phase-id)
                (map-insert
                  MintSessions
                  { owner: tx-sender, hash: expected-hash }
                  { fee-paid: true, phase-id: phase-id, created-at: stacks-block-height }
                )
                true
              )
            )
            (let ((mode (if (var-get allowlist-enabled) ALLOWLIST-GLOBAL ALLOWLIST-PUBLIC)))
              (begin
                ...
                (map-insert
                  MintSessions
                  { owner: tx-sender, hash: expected-hash }
                  { fee-paid: true, phase-id: u0, created-at: stacks-block-height }
                )
                true
              )
            )

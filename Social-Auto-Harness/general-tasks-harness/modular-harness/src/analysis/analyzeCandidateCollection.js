  ...
  return shortlisted.map((entry, shortlistIndex) => ({
    ...
    ...(candidateType === "thread"
      ...
      : {
          ...
          bio: entry.candidate?.bio ?? null,
          bioObserved: entry.candidate?.bioObserved ?? false,
          followers: entry.candidate?.followers ?? null,
          following: entry.candidate?.following ?? null,
          postCount: entry.candidate?.postCount ?? null,
          source: entry.candidate?.source ?? null,
        }),
...
export function buildCandidateAnalysisSynthesisPrompt({
  ...
  const selectiveGuidance =
    candidateType === "thread"
      ...
      : "Use the preliminary analysis as input, not as a mandate. Downgrade aggressively if a candidate is weakly relevant, low-signal, obviously spammy, or a poor fit for a cautious follow shortlist.";
  const diversityGuidance =
    candidateType === "thread"
      ? "Prefer breadth across authors and angles. Use prioritize very sparingly."
      : "Prefer breadth across different accounts and topical angles. Use prioritize very sparingly.";

...
test("analyzeCandidateCollection preserves follow-specific fields in the synthesis shortlist", async () => {
  let synthesisCall = null;
          ...
          analyses: [
            {
              ...
              xtrataFit: "high",
              suggestedAction: "follow",
              reasoning: "Strongest follow candidate.",
              draftBrief: null,
              riskFlags: [],
            },
          ],
  ...
  assert.match(synthesisCall.prompt, /"handle": "@builder"/);
  assert.match(synthesisCall.prompt, /"bio": "Stacks builder"/);
  assert.match(synthesisCall.prompt, /"followers": 120/);
  assert.equal(result.enrichedAnalyses[0].candidate.handle, "@builder");
});

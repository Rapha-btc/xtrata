...
export default function MarketScreen(props: MarketScreenProps) {
  ...
  const activeListingsQuery = useQuery({
    ...
    queryFn: async (): Promise<ActiveListing[]> => {
      ...
      while (
        cursor >= 0n &&
        scanned < ACTIVE_LISTINGS_SCAN_LIMIT &&
  ...
  const handleSelectActiveListing = (listing: ActiveListing) => {
    const value = listing.listingId.toString();
    setBuyListingTouched(true);
    setCancelListingTouched(true);
    setBuyListingIdInput(value);
    setCancelListingIdInput(value);
    setListingIdInput(value);
    setBuyStatus(null);
    setCancelStatus(null);
  };
        ...
        <div className="market-grid">
          ...
          <div className="market-block">
            ...
            {!activeListingsQuery.isFetching && activeListings.length === 0 && (
              <p className="field__hint">No active listings found.</p>
            )}
            {activeListings.length > 0 && (
              <>
                <p className="field__hint">
                  Tap a listing to fill the action forms.
                </p>
                ...
                <div className="market-listing-grid">
                  {activeListings.map((listing) => {
                    ...
                    return (
                      ...
                      >
                        <button
                          className="market-listing-card__frame"
                          type="button"
                          onClick={() => handleSelectActiveListing(listing)}
                        >

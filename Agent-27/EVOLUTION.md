...
- inscribe-entry.cjs (inscription mechanics)
...
### The 16KB boundary
- Currently hardcoded at 16,384 bytes in inscribe-entry.cjs and the compose prompt.
...
- **Local cache:** After each inscription, save the full HTML to a `history/` directory. The agent can read these during research pulses.
...
4. **Wallet autonomy** — The wallet is already the agent's. The mnemonic is in inscribe-entry.cjs. The agent could theoretically run from any machine that has the key and the code.

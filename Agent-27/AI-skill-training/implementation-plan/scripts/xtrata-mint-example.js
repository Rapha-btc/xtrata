 ...
 *
 * Usage:
 *   SENDER_KEY=<hex-private-key> node xtrata-mint-example.js <file-path> [mime-type]
 *
 * Example:
 *   SENDER_KEY=abc123... node xtrata-mint-example.js ./my-image.png image/png
 *
 * Requirements:
...
async function mint(filePath, mimeType) {
  ...
  const sealTx = await makeContractCall({
    ...
    functionArgs: [
      bufferCV(expectedHash),
      stringAsciiCV(DEFAULT_TOKEN_URI)
    ],
    senderKey,
    network,
    postConditions: [
...
const [,, filePath, mimeType = 'application/octet-stream'] = process.argv;
if (!filePath) {
  console.error('Usage: SENDER_KEY=<key> node xtrata-mint-example.js <file> [mime-type]');
  process.exit(1);
}

mint(filePath, mimeType).catch(err => {
